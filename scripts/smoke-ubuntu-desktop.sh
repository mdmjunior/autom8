...
log() {
  printf '\033[1;34m[AutoM8 Smoke]\033[0m %s\n' "$1"
}

warn() {
  printf '\033[1;33m[AutoM8 Smoke]\033[0m %s\n' "$1"
}

error() {
  printf '\033[1;31m[AutoM8 Smoke]\033[0m %s\n' "$1" >&2
}
...
detect_os() {
  ...
  if [[ "${ID:-}" != "ubuntu" ]]; then
    warn "Este smoke foi pensado para Ubuntu Desktop. Prosseguindo mesmo assim."
  fi
}

validate_required_commands() {
  log "Validando comandos básicos do ambiente."
  ...
  if [[ "$missing" -gt 0 ]]; then
    ...
    error "sudo apt update && sudo apt install -y git curl jq"
    exit 1
  fi
}
...
run_cli_smoke() {
  ...
  ./suite/bin/autom8 doctor || {
    warn "doctor retornou aviso/falha. Isso pode ser aceitável em VM sem todas as dependências do instalador oficial."
  }
  ...
  ./suite/bin/autom8 apps show mkpasswd
...
run_dry_run_smoke() {
  log "Executando dry-runs seguros."

  ./suite/bin/autom8 --dry-run clean
...
main() {
  log "Iniciando smoke test AutoM8 v0.2 em VM Ubuntu Desktop."
  ...
  echo "  Testar instalação real em uma VM descartável usando comandos específicos, não o install.sh público ainda."
}
