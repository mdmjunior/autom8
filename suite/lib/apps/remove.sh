...
autom8_apps_remove_category() {
  ...
  if [[ -z "$category" ]]; then
    category="$(autom8_apps_prompt_category "Selecione a categoria para remover" || true)"
  fi

  if [[ -z "$category" ]]; then
    autom8_warn_ui "Nenhuma categoria selecionada."
    autom8_summary_warn "Remoção por categoria sem seleção"
    return 0
  fi
  ...
  if ! autom8_is_supported_or_diagnostic_only; then
    autom8_error_ui "Distro não suportada oficialmente. Remoção por categoria bloqueada."
    autom8_summary_fail "Remoção por categoria bloqueada em distro não suportada"
    return 1
  fi
  ...
  autom8_header "Apps · remover categoria" "Remoção em lote com confirmação única."
  ...
  if [[ "${#ids[@]}" -eq 0 ]]; then
    ...
    autom8_summary_warn "Categoria sem apps removíveis"
    return 0
  fi

  echo
  if ! autom8_apps_validate_ids_have_packages "${ids[@]}"; then
    autom8_summary_fail "Remoção por categoria bloqueada por app inválido"
    return 1
  fi
  ...
  if [[ "${#installed_packages[@]}" -eq 0 ]]; then
    echo
    autom8_warn_ui "Nenhum pacote available desta categoria está instalado."
    autom8_summary_warn "Nada para remover na categoria"
    return 0
  fi
  ...
  if [[ "${AUTOM8_DRY_RUN:-false}" == "true" ]]; then
    echo
    autom8_success "Simulação concluída. Nada foi removido."
    autom8_summary_ok "Simulação de remoção por categoria concluída"
    return 0
  fi
  ...
  if ! autom8_confirm "Deseja remover os apps available desta categoria agora?"; then
    autom8_warn_ui "Remoção por categoria cancelada pelo usuário."
    autom8_summary_warn "Remoção por categoria cancelada"
    autom8_log_warn "apps" "Category removal canceled: $category"
    return 0
  fi

  autom8_require_admin "remover categoria de apps" || {
    autom8_summary_fail "Sem permissão para remover categoria"
    return 1
  }

  autom8_log_info "apps" "Removing category: $category apps: ${ids[*]} packages: ${installed_packages[*]}"

  if autom8_apps_remove_packages "${installed_packages[@]}"; then
    autom8_success "Categoria removida: $category"
    autom8_summary_ok "Categoria removida: $category"
  else
    autom8_error_ui "Falha ao remover categoria: $category"
    autom8_summary_fail "Falha na remoção por categoria"
    return 1
  fi
}
