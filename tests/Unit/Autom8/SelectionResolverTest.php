...
class SelectionResolverTest extends TestCase
{
    ...
    public function test_it_resolves_profiles_manual_packages_and_actions(): void
    {
        ...
        PackageVariant::create([
            'package_id' => $vlc->id,
            'distro_id' => $fedora->id,
            'install_command' => 'sudo dnf install -y vlc',
            'is_supported' => true,
        ]);
        ...
        $profile->packages()->attach($git->id, ['is_default' => true]);
        ...
        $this->assertSame(['git', 'vlc'], collect($result['packages'])->pluck('slug')->sort()->values()->all());
